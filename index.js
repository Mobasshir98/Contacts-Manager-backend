...
mongoose.connect(process.env.DATABASE_URL|| "mongodb+srv://10xacademy:10xacademy@cluster0.kstzf.mongodb.net/contactsmanager?retryWrites=true&w=majority").then(()=>{
    ...
    signupModal.find({ mailid: req.body.mailid }).then((userData) => {
        if (userData.length) {
            bcrypt.compare(req.body.password, userData[0].password).then((val) => {
                if (val) {
                    const authToken = jwt.sign(userData[0].mailid, process.env.SECRET_KEY);
    ...
    csv()
        ...
        .then(async (jsonObj) => {
            ...
            if(is_user.length>0){
                ...
                const uniqcontacts=[...new Map(newcontacts.map((item)=>[item["phonenumber"],item])).values()]
                usercontact.updateOne({user:user},{contacts:uniqcontacts}).then(()=>res.status(200).send("Updated Successfully")).catch((err)=>res.status(400).send(err))
                
            }
            else{
                const uniqcontacts=[...new Map(jsonObj.map((item)=>[item["phonenumber"],item])).values()]
                usercontact.create({
                    user,
...
app.delete('/delete', async (req,res)=>{
    try{
        ...
        const user=req.mailid;
        const deleted= await usercontact.updateOne({user:user},{$pull:{contacts:{_id:{$in:deleteitems}}}})
